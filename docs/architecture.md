...
production. The operator console in this repository is one of three delivery
...
```
                ┌─────────────────────────────────────────────────┐
                │                  Delivery                       │
                │   Operator console (this repo) · REST · MCP     │
                └────────────────────┬────────────────────────────┘
                                     │
┌──────────────┐    ┌────────────────▼─────────────────┐    ┌──────────────────┐
...
1. **Operator console** (this repository) — the daily UI for operations teams.
...
## 4. The operator console (this repository)

Next.js/TypeScript application. Relevant structure:

```
src/lib/engine-core/types.ts   Engine contract: Decision, Rule, Entity, widgets
src/lib/engine.ts              Engine facade: live (REST) and mock bindings
src/app/api/v1/                Console-hosted API surface (reason, rules, memory, audit)
src/components/                Agent thread, reasoning panel, rulebook, memory graph
```
